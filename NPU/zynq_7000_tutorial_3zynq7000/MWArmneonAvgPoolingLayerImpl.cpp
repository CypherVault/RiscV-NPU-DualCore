...
MWTargetNetworkImpl* ntwk_impl, int GDRXdUDklKFEYEfifhIH, int GLovsOhUpzOJhKgXUAJY, int 
GbdgxISzcqHOpzQEBrvP, int GmRRxuYauzGdhIlgciAT, int FOcStuqCptsGIZXskVpC, int 
DGzdAcREJHGXjyRzNjJV, int EfvWctmlsWAPsxXgdKWf, int EpwuhXsRcwdqXSjBpUeO, bool 
aPoolingIncludesPadding) : MWCNNLayerImpl{layer, ntwk_impl} , 
GIbahSoBBDrvvZduPEqU{GDRXdUDklKFEYEfifhIH}  , 
GLpnVFeGjOSrhNqnkdCu{GLovsOhUpzOJhKgXUAJY}  , 
...
, EGsHUnogBQpOwCZJYeUd{FOcStuqCptsGIZXskVpC} , 
DRzwhbNPpftRRIXXfHzd{DGzdAcREJHGXjyRzNjJV} , 
FkIKxivEqRgnWINQcvLh{EfvWctmlsWAPsxXgdKWf} , 
FshVHIJMRAhtQirYPlZd{EpwuhXsRcwdqXSjBpUeO} , 
nrUaguRlWYtZgNEGzlhH{aPoolingIncludesPadding} { 
setupIpArmTensors(); 
...
if ((GIbahSoBBDrvvZduPEqU == -1) && (GLpnVFeGjOSrhNqnkdCu == -1)) { 
GDRXdUDklKFEYEfifhIH = ipTensor->getHeight() + EGsHUnogBQpOwCZJYeUd + 
DRzwhbNPpftRRIXXfHzd; GLovsOhUpzOJhKgXUAJY = ipTensor->getWidth() + 
FkIKxivEqRgnWINQcvLh + FshVHIJMRAhtQirYPlZd ; } configureIpArmTensors(); 
arm_compute::Tensor* prevLayerarmTensor = getCurrentLayerIpArmTensor(); if 
(opTensor->isFloat()) { 
...
arm_compute::Size2D(GLovsOhUpzOJhKgXUAJY, GDRXdUDklKFEYEfifhIH), 
arm_compute::DataLayout::NCHW, arm_compute::PadStrideInfo(GmRRxuYauzGdhIlgciAT, 
GbdgxISzcqHOpzQEBrvP, FkIKxivEqRgnWINQcvLh, FshVHIJMRAhtQirYPlZd, 
EGsHUnogBQpOwCZJYeUd, DRzwhbNPpftRRIXXfHzd, 
arm_compute::DimensionRoundingType::FLOOR), !nrUaguRlWYtZgNEGzlhH));
#else
 m_avgPoolLayer->configure( prevLayerarmTensor, getCurrentLayerOpArmTensor(), 
arm_compute::PoolingLayerInfo( arm_compute::PoolingType::AVG, 
arm_compute::Size2D(GLovsOhUpzOJhKgXUAJY, GDRXdUDklKFEYEfifhIH), 
...
DRzwhbNPpftRRIXXfHzd, arm_compute::DimensionRoundingType::FLOOR), !nrUaguRlWYtZgNEGzlhH));
#endif
